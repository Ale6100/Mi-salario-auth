// src\components\Page\Configuration\IncomeSources\DialogAddEditIncomeSource.tsx

import { Button } from "@/components/ui/button";
import { Controller, useForm, useWatch } from "react-hook-form";
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { fetchPostFuenteIngresos, fetchPutFuenteIngresos } from "@/lib/fetch/fuentesIngresos";
...
type DialogAddEditIncomeSourceProps = {
  readonly isOpen: { status: boolean, source: FuenteIngresosDB | undefined };
  readonly setIsOpen: Dispatch<SetStateAction<{ status: boolean, source: FuenteIngresosDB | undefined }>>;
  readonly actualSources: FuenteIngresosDB[];
}

export const DialogAddEditIncomeSource = ({ isOpen, setIsOpen, actualSources }: DialogAddEditIncomeSourceProps) => {
  ...
  const aguinaldo = useWatch({ control: form.control, name: "aguinaldo" });
  ...
  const onSubmit = async (data: FormSchema) => {
    ...
    if (response.statusCode != 201) {
      toast.error("Error al crear la fuente de ingreso", { id: toastId });
      return close();
    }

    await queryClient.invalidateQueries({ queryKey: ['fuentes-ingreso'] });
    toast.success(`Fuente de ingreso creada con éxito`, { id: toastId });
    close();
  }
  ...
  const onEdit = async (data: FormSchema) => {
    ...
    toast.loading("Espere...", { id: toastId });
    const response = await fetchPutFuenteIngresos({
      token,
      id: isOpen.source._id,
      data: dataToSend,
    })

    if (response.statusCode != 200) {
      toast.error("Error al editar la fuente de ingreso", { id: toastId });
      return close();
    }

    await queryClient.invalidateQueries({ queryKey: ['fuentes-ingreso'] });
    toast.success(`Fuente de ingreso editada con éxito`, { id: toastId });
    close();
  }
  ...
  const nombreContieneAguinldoPeroNoEsAguinaldo = nombre?.toLowerCase().includes("aguinaldo") && !aguinaldo;
  ...
  return (
    <Dialog open={isOpen.status} onOpenChange={open => {
      ...
      <DialogContent className={'max-w-[95vw]! sm:max-w-[80vw]! max-h-[90dvh] overflow-x-hidden'}>
        <DialogHeader>
          <DialogTitle className="sm:text-lg">{isEdit ? "Editar fuente de ingreso" : "Agregar fuente de ingreso"}</DialogTitle>
          <DialogDescription />
        </DialogHeader>
        <section>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    />
                    {nombreContieneAguinldoPeroNoEsAguinaldo && (
                      <p className="text-sm text-muted-foreground">
                        Si la fuente corresponde a un aguinaldo, se recomienda marcarla explicitamente como tal en el switch de abajo
                      </p>
                    )}
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <div className="flex items-center gap-3">
                      <FieldLabel className="required mb-0 cursor-pointer" htmlFor="aguinaldo">
                        Esta fuente es un aguinaldo:
                      </FieldLabel>
                      <Switch
                ...
                !aguinaldo && (
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <Field data-invalid={fieldState.invalid}>
                        <div className="flex items-center gap-3">
                          <FieldLabel className="required mb-0 cursor-pointer" htmlFor="activo">
                            Recibo ingresos de esta fuente actualmente:
                          </FieldLabel>
                          <Switch
                            className="cursor-pointer"
                            checked={field.value}
